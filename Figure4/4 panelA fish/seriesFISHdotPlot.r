...
#load input files
DMSO_FISH <- read.csv("231inhibitorCd-dmsoNoCd.csv", sep=",")
DMSO_FISH$dose <- as.factor(cadR231series$dose)

#make dot plot of FISH data series   
ggplot(DMSO_FISH, aes(dose, green_red, fill = count, color = count)) + 
...
#save the files
ggsave("4A.tiff", device="tiff", width =3.53, height =2.61 , units ="in" , dpi=300)
