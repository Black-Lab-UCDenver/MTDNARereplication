...
#load input files
cadR10_UNC <- read.csv("231inhibitorCd-unc10uMCd.csv", sep=",")
cadR10_UNC$dose <- as.factor(cadR231series$dose)

#make dot plot of FISH data series   
ggplot(cadR10_UNC, aes(dose, green_red, fill = count, color = count)) + 
...
#save the files
ggsave("4D.tiff", device="tiff", width =3.53, height =2.61 , units ="in" , dpi=300)
