...
#load input files
UNC_FISH <- read.csv("231inhibitorCd-uncNoCd.csv", sep=",")
UNC_FISH$dose <- as.factor(cadR231series$dose)

#make dot plot of FISH data series   
ggplot(UNC_FISH, aes(dose, green_red, fill = count, color = count)) + 
...
#save the files
ggsave("4B.tiff", device="tiff", width =3.53, height =2.61 , units ="in" , dpi=300)
