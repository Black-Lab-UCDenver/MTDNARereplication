...
#load input files
VE821redo_UNC_FISH <- read.csv("231inhibitorVE821-1stsetredo-unc.csv", sep=",")
VE821redo_UNC_FISH$dose <- as.factor(VE821redo_UNC_FISH$dose)

#make dot plot of FISH data series   
ggplot(VE821redo_UNC_FISH, aes(dose, green_red, fill = count, color = count)) + 
...
#save the files
ggsave("S5D.tiff", device="tiff", width =3.53, height =2.61 , units ="in" , dpi=300)
