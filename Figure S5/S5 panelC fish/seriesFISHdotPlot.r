...
#load input files
VE821redo_DMSO_FISH <- read.csv("231inhibitorVE821-1stsetredo-dmso.csv", sep=",")
VE821redo_DMSO_FISH$dose <- as.factor(VE821redo_DMSO_FISH$dose)

#make dot plot of FISH data series   
ggplot(VE821redo_DMSO_FISH, aes(dose, green_red, fill = count, color = count)) + 
...
scale_y_continuous(limits=c(2.1,6), 
...
#save the files
ggsave("S5C.tiff", device="tiff", width =3.53, height =2.61 , units ="in" , dpi=300)
