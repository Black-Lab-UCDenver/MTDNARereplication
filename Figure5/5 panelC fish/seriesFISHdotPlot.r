...
#load input files
VE821ser_UNC_FISH <- read.csv("231inhibitorVE821-unc.csv", sep=",")
VE821ser_UNC_FISH$dose <- as.factor(VE821ser_UNC_FISH$dose)

#make dot plot of FISH data series   
ggplot(VE821ser_UNC_FISH, aes(dose, green_red, fill = count, color = count)) + 
...
#save the files
ggsave("5C.tiff", device="tiff", width =3.53, height =2.61 , units ="in" , dpi=300)
