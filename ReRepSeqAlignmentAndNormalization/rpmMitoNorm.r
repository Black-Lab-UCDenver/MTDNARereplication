...
dfIn <- read_tsv("sample.bl.bg", col_names = FALSE) 
dfCounts <- read_tsv("Sample.stats.txt", col_names = FALSE)
# dfBL <- read_tsv("blackListStats.txt", col_names = FALSE)
...
dfIn$X5 <- dfIn$X4/((totalCount)/1000000)

# Perform the mitochondrial DNA scaling function. 
dfIn$X6 <- dfIn$X5/(totalM/(totalCount))
